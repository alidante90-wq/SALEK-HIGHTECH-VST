...
public:
    ...
    void setPm2to1(float v); void setPm3to1(float v); void setAm2to1(float v); void setRm2to1(float v);
    void setFilterCutoff(float v); void setFilterResonance(float v); void setFilterDrive(float v);
    void setFilterMode(int v); void setFilterEnvAmt(float v);
    void setAmpAttack(float v); void setAmpDecay(float v); void setAmpSustain(float v); void setAmpRelease(float v);
...
private:
    ...
    ModMatrix modMatrix;
    template<typename Fn> void forEachVoice(Fn&& fn) {
        for (int i = 0; i < synth.getNumVoices(); ++i)
            if (auto* v = dynamic_cast<SynthVoice*>(synth.getVoice(i))) fn(*v);
    }
};
}
