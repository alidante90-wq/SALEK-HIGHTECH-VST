...
public:
                           ...
                           juce::Slider& slider) override
    {
        ...
        for (int i = 3; i >= 1; --i)
        {
            ...
            g.drawEllipse (cx - radius - i * 2.5f, cy - radius - i * 2.5f,
                           (radius + i * 2.5f) * 2, (radius + i * 2.5f) * 2, 2.0f);
        }

        juce::ColourGradient body (juce::Colour (0xff1a1a28), cx, cy - radius,
                                   juce::Colour (0xff08080f), cx, cy + radius, false);
        ...
        juce::Path needle;
        needle.addRoundedRectangle (-1.8f, -radius * 0.72f, 3.6f, radius * 0.48f, 1.0f);
...
public:
    ...
    void paint (juce::Graphics& g) override
    {
        ...
        g.setColour (juce::Colour (0xff00f0ff).withAlpha (0.08f));
        for (int i = 1; i < 4; ++i)
            g.drawHorizontalLine ((int) (r.getY() + r.getHeight() * i / 4), r.getX(), r.getRight());

        juce::Path wave;
        for (int i = 0; i < 64; ++i)
        {
            float t = (float) i / 63.0f;
            float phase = t * juce::MathConstants<float>::twoPi * 3.0f + phaseOffset;
            float y = std::sin (phase) * 0.55f + 0.15f * std::sin (phase * 3.0f + 1.0f);
